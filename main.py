...
def generate_reports():
    ...
    status_code, get_data_elements = http_report_service.get_data("dataElements", 2000000)
    print("Pulling Category Option Combos from server")
    status_code, category_option_combos = http_report_service.get_data("categoryOptionCombos", 2000000)
    print("Pulling Organization Units")
    status_code, organization_units = http_report_service.get_data("organisationUnits", 2000000)
    organization_units = organization_units["organisationUnits"]
    organization_units_df = pd.DataFrame.from_dict(organization_units)
    organization_units_df.rename(columns={'displayName': 'name'}, inplace=True)
    print("Pulling reports from server has started")
    http_report_service.get_reports_from_server(organization_units_df)
